...
from .periodic_backup import (
    ...
    suspend_periodic_backup_source,
    ...
    def _rebind_storage_for_home(self, hermes_home: str = "") -> bool:
        ...
        if self._config.database_path:
            ...
            if current_home == str(hermes_home) and current_store_home == str(hermes_home):
                return False
            # Keep the current lease registered until the replacement has been
            # admitted against this same configured source.  Releasing first
            # can remove a single-owner source after its idle worker exits,
            # allowing a different home/root to start a fresh ACTIVE source
            # without the required ambiguity and historical-readmission gate.
            previous_registration = self._periodic_backup_registration
            self._hermes_home = hermes_home
            store = getattr(self, "_store", None)
            if store is not None:
                store._hermes_home = hermes_home
            self._reset_profile_runtime_state()
            replacement_registration = register_periodic_backup(self)
            if replacement_registration.state == "error":
                # A missing or unsafe replacement root is still a root-authority
                # transition for this configured database.  Keep and suspend the
                # prior lease rather than unregistering it: otherwise a later
                # valid retry can create a fresh source and publish without the
                # required ambiguity/readmission check.
